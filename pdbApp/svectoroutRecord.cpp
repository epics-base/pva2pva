...
long type_check(svectoroutRecord *prec)
{
...
{
    svectoroutRecord *prec = (svectoroutRecord*)pcommon;
    svectoroutdset *pdset = (svectoroutdset *)(prec->dset);
    ...
    if(pass==0) {
        ...
        if (!pdset) {
            ...
            recGblRecordError(S_dev_noDSET, prec, "svectorout: no DSET");
            return S_dev_noDSET;
        }
    ...
    #undef CASE
        ...
        default:
            ...
            recGblRecordError(S_db_badDbrtype, prec, "svectorout: bad FTVL");
            return S_db_badDbrtype;
        }
...
long readValue(svectoroutRecord *prec,
               svectoroutdset *pdset)
{
    long ret = pdset->write_sharedvector(prec);
...
void monitor(svectoroutRecord *prec)
{
...
long process(struct dbCommon *pcommon)
{
    ...
    if( (pdset==NULL) || (pdset->write_sharedvector==NULL) ) {
        prec->pact=TRUE;
        recGblSetSevrMsg(prec, READ_ALARM, INVALID_ALARM, "no read_svectorout");
        recGblRecordError(S_dev_missingSup,(void *)prec,"read_svectorout");
...
{
    svectoroutRecord *prec = (svectoroutRecord*)paddr->precord;
...
long get_array_info(DBADDR *paddr, long *no_elements, long *offset)
{
    ...
    *offset = 0;
    if(dbGetFieldIndex(paddr)==svectoroutRecordVAL) {
...
extern "C" {
epicsExportAddress(rset,svectoroutRSET);
}
